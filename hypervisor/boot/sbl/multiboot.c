	...
	if (boot_regs[0] != MULTIBOOT_INFO_MAGIC) {
		...
		if (mbi != NULL) {
			...
			if ((mbi->mi_flags & MULTIBOOT_INFO_HAS_MODS) == 0U) {
				...
				vm->sw.linux_info.bootargs_src_addr = (void *)vm->vm_config->os_config.bootargs;
				vm->sw.linux_info.bootargs_size = strnlen_s(vm->vm_config->os_config.bootargs, MEM_2K);
				vm->sw.linux_info.bootargs_load_addr = (void *)(vm->vm_config->memory.size -  8*1024UL);
				clac();
				ret = 0;
			}
		}
